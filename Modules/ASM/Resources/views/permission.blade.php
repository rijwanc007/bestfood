        ...
        <div class="card card-custom gutter-b">
            <div class="card-header flex-wrap py-5">
                ...
                <div class="card-toolbar">
                    <!--begin::Button-->
                    <a href="{{ route('asm') }}" class="btn btn-secondary btn-sm font-weight-bolder"> 
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <form id="saveDataForm" method="post">
                            @csrf 
                            <div class="row">
                                <input type="hidden" name="asm_id" value="@isset($asm){{$asm->id}}@endisset" id="asm_id">
                                <div class="form-group col-md-12 required">
                                    <label for="role_name">ASM Name</label>
                                    <input type="text" class="form-control" value="@isset($asm){{$asm->name}}@endisset">
                                </div>
                                <div class="col-md-12">
                                            ...
                                            @foreach ($permission_modules as $menu)
                                                @if ($menu->submenu->isEmpty())
                                                    <li>
                                                        ...
                                                        @isset($asm_module) @if(collect($asm_module)->contains($menu->id)) {{ 'checked' }} @endif @endisset > 
                                                            ...
                                                            <ul>
                                                                @foreach ($menu->permission as $permission)
                                                                    <li><input type="checkbox" name="permission[]" value="{{ $permission->id }}"
                                                                        @isset($asm_permission) @if(collect($asm_permission)->contains($permission->id)) {{ 'checked' }} @endif @endisset />
                                                                            {{ $permission->name }}
                                                                    </li>
                                                                @endforeach
                                                            </ul>
                                                        ...
                                                        @foreach ($menu->submenu as $submenu)
                                                            @if ($submenu->submenu->isEmpty())
                                                                <li>
                                                                    ...
                                                                    @isset($asm_module) @if(collect($asm_module)->contains($submenu->id)) {{ 'checked' }} @endif @endisset >
                                                                        {{ $submenu->module_name }}
                                                            ...
                                                            <li>
                                                                ...
                                                                <ul>
                                                                    ...
                                                                    <li>
                                                                        ...
                                                                        @isset($asm_module) @if(collect($asm_module)->contains($sub_submenu->id)) {{ 'checked' }} @endif @endisset >
                                ...
                                <div class="col-md-12 pt-4 text-center">
                                    <a href="{{ route('asm') }}" class="btn btn-danger btn-sm font-weight-bolder">Cancel</a>
...
$(document).ready(function(){
    ...
    $(document).on('click', '#save-btn', function () {
        ...
        if($('.module:checked').length >= 1){
            $.ajax({
                url: "{{route('asm.permission.store')}}",
                type: "POST",
                data: formData,
                dataType: "JSON",
                contentType: false,
                ...
                success: function (data) {
                    ...
                    if (data.status == false) {
                        ...
                        if (data.status == 'success') {
                            window.location.replace("{{ url('assistant-sales-manager/permission',$asm->id) }}");
                        }
                    }
