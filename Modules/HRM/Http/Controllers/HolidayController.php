...
class HolidayController extends BaseController
{
    ...
    public function index()
    {
        if(permission('holiday-access')){
            $this->setPageData('Holiday','Holiday','fas fa-warehouse',[['name'=>'HRM','link'=>'javascript::void();'],['name' => 'Holiday']]);
            $deletable = self::DELETABLE;
            return view('hrm::holiday.index',compact('deletable'));
        }else{
        ...
        if($request->ajax()){
            if(permission('holiday-access')){
                ...
                foreach ($list as $value) {
                    ...
                    $row[] = $value->end_date;
                    $row[] = permission('holiday-edit') ? change_status($value->id,$value->status, $value->name) : STATUS_LABEL[$value->status];
    ...
    public function store_or_update_data(HolidayFormRequest $request)
    {
        if($request->ajax()){
